...
std::string buildProbeMatch(const std::string& relates_to_uuid,
    ...
    oss << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
        ...
        <<   "<wsa:RelatesTo>" << xmlEscape(relates_to_uuid) << "</wsa:RelatesTo>"
        ...
        <<         "<wsa:Address>" << xmlEscape(device_endpoint_uuid) << "</wsa:Address>"
        <<       "</wsa:EndpointReference>"
        <<       "<wsd:Types>dn:NetworkVideoTransmitter tds:Device</wsd:Types>"
        <<       "<wsd:Scopes>" << xmlEscape(scopes) << "</wsd:Scopes>"
        <<       "<wsd:XAddrs>" << xmlEscape(xaddr) << "</wsd:XAddrs>"
